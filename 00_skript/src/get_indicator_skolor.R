...
hpl = read.csv2(paste0("00_skript/data/interim/AllaLinjerPerHplID_", hpl_file_date,".csv"))

skol=readOGR("00_skript/data/input/shp/skolor/Buffer_of_Skolor_20200518.shp", 
...
ptm <- proc.time()
latest_row <- 1
while(latest_row <= nrow(df4)){
  
  last_row <- min(nrow(df4), latest_row + 9)
  x <- lapply(df4$url[latest_row:last_row], get_JSON_data_from_UL_API)
...
write.csv2(fin, "00_skript/data/interim/skol.csv", row.names = F)
...
fin1 = fin %>% distinct() 
write.csv2(fin1, "00_skript/data/interim/skol1.csv", row.names = F)
...
write.csv2(fin2, "00_skript/data/interim/skol2.csv", row.names = F)
...
write.csv2(fin4, "00_skript/data/output/skolor.csv", row.names = F)
write.csv2(SkolSummary, "00_skript/data/output/skolor_ResultatKort.csv", row.names = F)
