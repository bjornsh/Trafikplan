...
dat = read.csv2(paste0(getwd(), "/00_skript/data/output/Resultat_2021-05-11.csv"))
tatort <- sf::st_read(paste0(getwd(), "/00_skript/data/input/shp/To2018_Swe99TM.shp"), options="UTF-8", quiet = TRUE)
## byt CRS fran Sweref90 TM till WGS84
# st_crs(tatort)
```
...
```{r}
tatort_include = c("Stockholm", "Gävle", "Västeras")
tatort_upps = tatort %>% 
...
leaflet(dat_stop) %>%
    ...
    addPolygons(fillColor = ~pal(Lagtrafiktimmar),
                ...
                fillOpacity = 1, 
                group = "stop",
                popup = ~paste(paste0("Lagtrafiktimmar = ", round(100*Lagtrafiktimmar,0)),"%")) %>%   
    ...
    addLegend(pal = pal,
              ...
              title = "Lagtrafiktimmar", position = "bottomright") %>% 
